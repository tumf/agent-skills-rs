...
- [x] GitHub ソースの discover→install→lock(global) が CLI 実フローで未接続です。`install_skill_command` が `SourceType::Github` でも `Source { url: None }` を生成し `discover_skills(&source, ...)` を直接呼ぶため（`src/bin/my_command.rs:114`-`src/bin/my_command.rs:120`、`src/bin/my_command.rs:134`-`src/bin/my_command.rs:136`）、`discover_skills_with_provider` の provider 経路に到達せず `discover_skills` は空配列を返して `No skills found.` で終了します（`src/discovery.rs:58`-`src/discovery.rs:76`、実行確認: `cargo run --bin my-command -- install-skill github --yes`）。CLI から provider 抽象を注入して GitHub 経路を実フローに組み込み、global 時は provider ハッシュで `update_entry_with_hash` を呼ぶ統合テストを追加してください。
